...
export default function Home({ session }) {
  ...
  // Mock data from your HTML example
  const mockStories = [
  ...
  function openLiveNow() {
    ...
    modal.innerHTML = `
      <div class="modal-content" style="max-width: 500px;">
        <div class="modal-title">🔴 Live Now</div>
        <div id="liveStreamsList">
          ${mockLiveStreams.map(live => `
            <div style="display: flex; align-items: center; gap: 12px; padding: 12px; border-bottom: 1px solid #eee; cursor: pointer;" onclick="window.location.href='/live'">
              <div style="width: 50px; height: 50px; border-radius: 50%; background: linear-gradient(135deg, #667eea, #764ba2); display: flex; align-items: center; justify-content: center; color: white; font-weight: bold;">${live.avatar}</div>
              <div style="flex: 1;">
                <div style="font-weight: bold;">🔴 ${live.host}</div>
                <div style="font-size: 12px; color: #666;">${live.title}</div>
                <div style="font-size: 11px; color: #999;">${live.viewers} viewers</div>
              </div>
            </div>
          `).join('')}
        </div>
        <button class="secondary-btn" style="margin-top: 16px; width: 100%;" onclick="this.closest('.modal').remove()">Close</button>
      </div>
    `
    ...
    if (activeStoryTab === 'stories') {
      ...
      return (
        <>
          ...
          <div className="story-card" onClick={openLiveNow} style={{ background: 'linear-gradient(135deg, #ff6b6b 0%, #ee5a52 100%)' }}>
            <div className="story-avatar" style={{ fontSize: '30px' }}>📺</div>
    ...
    <Layout session={session}>
      ...
      <div className="create-post-box" onClick={() => setShowCreatePost(true)}>
        ...
        <div className="post-actions-row">
          <div className="post-action" onClick={() => alert('Go Live feature coming soon')} title="Go Live & Notify Friends">
            <i className="fas fa-circle" style={{ color: '#f5576c' }}></i> Live
          </div>
          <div className="post-action" onClick={() => document.getElementById('postImageInput').click()}>
            <i className="fas fa-image"></i> Photo
          </div>
          <div className="post-action" onClick={() => alert('Upload music coming soon')}>
          ...
          <div className="modal-content" style={{ maxWidth: '550px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
              <div className="post-avatar" style={{ width: '40px', height: '40px' }}>
                <img src={session?.user?.user_metadata?.avatar_url || `https://ui-avatars.com/api/?name=${session?.user?.email?.[0] || 'U'}&background=000&color=fff`} alt="" style={{ width: '100%', height: '100%', borderRadius: '50%' }} />
              </div>
              <div>
